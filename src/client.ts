...
class Client {
  ...
  readonly baseEndpoint = 'https://snip.hxrsh.in/api'
  ...
  async post<T>(url: string, body?: unknown): Promise<T | undefined> {
    return this.successOrUndefined(await this.makeRequest(url, 'POST', body))
  }
  ...
  async patch<T>(url: string, body?: unknown): Promise<T | undefined> {
    return this.successOrUndefined(await this.makeRequest(url, 'PATCH', body))
  }
